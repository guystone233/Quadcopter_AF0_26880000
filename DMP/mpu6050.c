...
//            Yaw = data[2] *1.0 /(1<<16);
//            USART1_printf("Pitch : %.4f  ",Pitch );
//						USART1_printf("Roll :  %.4f  ",Roll );
...
{ 
  ...
  if (result) {
      USART1_printf("Could not initialize gyro.\n");
  }
  ...
  result = inv_init_mpl();
  if (result) {
      USART1_printf("Could not initialize MPL.\n");
  }
  ...
  if (result == INV_ERROR_NOT_AUTHORIZED) {
      while (1) {
          USART1_printf("Not authorized.\n");
      }
  }
  if (result) {
      USART1_printf("Could not start the MPL.\n");
  }
